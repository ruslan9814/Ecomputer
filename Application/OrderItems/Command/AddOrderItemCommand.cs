...
internal sealed class AddOrderItemCommandHandler(IOrderRepository orderRepository, IProductRepository productRepository,
    IUnitOfWork unitOfWork) : IRequestHandler<AddOrderItemCommand, Result>
{
    ...
    public async Task<Result> Handle(AddOrderItemCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            var result = orderItem.AddProductToOrderItem(request.OrderId, request.ProductId, request.Quantity, request.Price);
            if (result.IsFailure)
            {
